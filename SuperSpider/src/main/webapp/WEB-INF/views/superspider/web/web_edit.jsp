...
<div id="pagelayout" class="easyui-layout" style="overflow: none;border:none;/* max-height:495px; */" data-options="fit:true">
	...
	<div class="easyui-panel" id="mainlayout" style="overflow: auto;border:none;/* max-heighta:495px; */" data-options="region:'center'">
		<form id="configForm" action="" method="post">
			...
			<div id="regex" class="easyui-panel" title="采集规则设置" style="height: 600px;/* overflow-y :auto; border:5px solid red;*/" data-options="maximizable:true,collapsible:true">
...
function saveWebconfig()
{
	...
	getTabIndex();
	//var webTypeRuleRows= dg_type1.datagrid('getRows');
...
// 	var currentType = "";
// 	currentType = $("#typeList").combobox('getValue');
// 	if(currentType!="" && currentType!=null && currentType!="null" && currentType!="undefined")
// 	{
// 		storeParamObjectTemp(currentType)
// 	}
	//alert(paramObjectList.length)
	var paramJson =$.toJSON(paramObjectList);
	var paramObjectListJson = $("#paramObjectListJson");
	paramObjectListJson.val(paramJson);
	
// 	var fieldMappingJson = $.toJSON($("#webfieldmapping").datagrid('getRows'));
	getFieldMappingList();
	...
	var url = '${pageContext.request.contextPath}/webController/edit';
...
function loadcompleted(panelid, flag) {
	...
	$("#mainlayout").panel({
		onResize: function () {
			...
			$("#dg_type2").datagrid('resize',{width:w-10});
...
$(function(){
	...
	$("#layout_split_center").panel({
		onResize:function(width, height)
		{
			...
			//alert("regex_jh="+regex_jh+",north_h="+north_h+",height="+height)

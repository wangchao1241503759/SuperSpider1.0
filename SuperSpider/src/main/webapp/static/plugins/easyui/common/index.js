...
(function ($) {
    ...
    window.mainpage.mainTabs.isExists = function (title, href) {
        var t = $(mainTabs), tabs = t.tabs("tabs"), array = $.array.map(tabs, function (val) { return val.panel("options"); }),
            list = $.array.filter(array, function (val) {/*alert(val.title+"==="+title); */return val.title == title; }), ret = list.length ? 1 : 0;
        if (ret && $.array.some(list, function (val) {/*alert(val.href+"==="+href); */return val.href == href; })) { ret = 2; }
        return ret;
    };

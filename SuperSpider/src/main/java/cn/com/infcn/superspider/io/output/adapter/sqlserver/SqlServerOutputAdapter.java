 ...
 * 
 */
package cn.com.infcn.superspider.io.output.adapter.sqlserver;
 ...
 */
public class SqlServerOutputAdapter extends OutputAdapter{

	Logger logger = Logger.getLogger(jdbcOutputAdapter.class);
	private BasicDao basicDao = null;
	
	public SqlServerOutputAdapter(){}
	public SqlServerOutputAdapter(JSONObject obj, Site site) throws Throwable {
		this.site = site;
		init(obj);
	}
	public SqlServerOutputAdapter(JSONObject obj, Site site ,SpiderListener listener) throws Throwable {
		this(obj,site);
	...
	public void CreateTable(JSONObject tableStruct)
	{
		...
		try {
			...
			{
				String deleteSql = "if exists ( select * from INFORMATION_SCHEMA.tables where table_name = '"+dbModel.getTablename()+"') drop table \""+dbModel.getTablename()+"\"";
	...
	public void shutdown(boolean isShutdownNow) {
		try {
			...
			logger.error("["+this.site.getName()+"]"+Thread.currentThread()+"SqlServerOutputAdapter.name->"+site.getName()+".ThreadPool shutdown failed.",e);
		}finally{
			if(queue != null)
			{

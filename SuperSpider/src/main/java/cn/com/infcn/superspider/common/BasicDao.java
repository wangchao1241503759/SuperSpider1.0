...
public class BasicDao {
	...
	public BasicDao(String driverClassName, String jdbcUrl, String username, String password) {
		...
		ds.setMaxWait(3000);
		// 设置是否自动提交事务,批量插入时:要设置为false，效率很高，否则很慢;
	 ...
	 * @param json
	 */
	public void save(String table, JSONObject json) throws Exception {
		Chain chain = null;
	 ...
	 * @param json
	 */
	public void update(String table, JSONObject json,String primaryField) throws Exception {
		Chain chain = null;
		for (Entry<String, Object> entry : json.entrySet()) {
			if (chain == null) {
				chain = Chain.make(entry.getKey(), entry.getValue());
		...
		dao.update(table, chain,Cnd.where(primaryField,"=",json.get(primaryField)));
	}
	/**
	 * 查询数据库中的数据条数
	 * 
	 ...
	 * @param sqls
	 */
	public <T> void executeSql(Sql... sqls) throws Exception {
		for (Sql sql : sqls) {
			dao.execute(sql);
		}
	}
	...
	public void deleteByIds(String table, String idName, List<Object> ids) {
		...
		this.executeSql("DELETE FROM "+table +" WHERE "+idName +" IN ("+idsStr+")");
	}
}
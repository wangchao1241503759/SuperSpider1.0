 ...
 * 
 */
package cn.com.infcn.superspider.io.output.adapter.db2;
 ...
 */
public class Db2OutputAdapter extends OutputAdapter{

	Logger logger = Logger.getLogger(jdbcOutputAdapter.class);
	private BasicDao basicDao = null;
	
	public Db2OutputAdapter(){}
	public Db2OutputAdapter(JSONObject obj, Site site) throws Throwable {
		this.site = site;
		init(obj);
	}
	public Db2OutputAdapter(JSONObject obj, Site site ,SpiderListener listener) throws Throwable {
		this(obj,site);
	...
	public void CreateTable(JSONObject tableStruct)
	{
		...
		try {
			if("1".equals(is_clear))
			{
				...
				sqlStr.append("select 1 from syscat.tables where tabname = upper('"+dbModel.getTablename()+"')");
	...
	public void shutdown(boolean isShutdownNow) {
		try {
			...
			logger.error("["+this.site.getName()+"]"+Thread.currentThread()+"Db2OutputAdapter.name->"+site.getName()+".ThreadPool shutdown failed.",e);
		}finally{
			if(queue != null)
			{

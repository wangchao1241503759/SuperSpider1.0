...
public class FtpSpiderListener extends SpiderListenerAdaptor {
	...
	private final static Logger logger = Logger.getLogger(FtpSpiderListener.class);
	...
	public void onStartup(Site site) {
        ...
        try
        {
			...
			// 添加任务日志
			ListenerUtil.addLogByStart(ConfigUtil.get("ftpModule"), site.getName(), taskLogService, site.getId(), task.getStartCount());
			OutputDispatcher Output = OutputDispatcher.me().init(site,this).startup();// 启动输出适配器;
			OutputDispatcherFactory.getInstance().addOutput(site, Output);// 添加任务输出适配器;
	...
	public void onAfterShutdown(Site site, Object... args) {
		try{
			...
			//添加任务日志
	 		ListenerUtil.addLogByFinishNum(ConfigUtil.get("ftpModule"), site.getName(), site.getId(), taskLogService, taskService,Integer.parseInt(site.getOption(Constant.START_COUNT).toString()));
	 		ListenerUtil.addLogByEnd(ConfigUtil.get("ftpModule"), site.getName(), taskLogService,site.getId(),Integer.parseInt(site.getOption(Constant.START_COUNT).toString()));
		}catch (Exception err)
        {
			String errorInfo = err.getCause().toString();
			logger.error(errorInfo, err);
	...
	public void onError(Thread thread, Task task, String err, Throwable e) {
		...
		logs.offer("<font color='#FF0000'>[ERROR] ~" + err + "</font>");
  		//添加任务日志
		if(task == null){
			ListenerUtil.addLogByError(ConfigUtil.get("ftpModule"), this.site.getName(), err, taskLogService,site.getId(),Integer.parseInt(site.getOption(Constant.START_COUNT).toString()));
		}
	}

	@Override
	public void onParse(Thread thread, Task task,List<Map<String, Object>> models) {
		if(ModelParseUtil.getLocalFile(task).isFile()){//当前目标是文件才进行提取
			...
			try{
				...
				// 调用输出数据存储模块进行数据持久化存储;
				if (validOutputStart()) {
					OutputDispatcher outputDispatcher = OutputDispatcherFactory.getInstance().getOutput(task.site);
					// 调用模型转换工具将models转换为业务相关数据;
					JSONArray jsonArray = ModelParseUtil.toFileJsonData(task,models);
					Doc doc = ModelParseUtil.toDoc(outputDispatcher, jsonArray,task.target);
					if (doc != null)
						outputDispatcher.pushQueue(doc); 
				}
